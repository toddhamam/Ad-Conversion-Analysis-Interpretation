  ...
  try {
    switch (route) {
      ...
      case 'youtube-transcript':
        return handleYouTubeTranscript(req, res);
      case 'video-direct-start':
        return handleVideoDirectStart(req, res);
      case 'video-direct-transfer':
        return handleVideoDirectTransfer(req, res);
      case 'video-direct-finish':
        return handleVideoDirectFinish(req, res);
...
async function handleYouTubeTranscript(req: VercelRequest, res: VercelResponse) {
  ...
  if (!rawVideoId || typeof rawVideoId !== 'string') return res.status(400).json({ error: 'videoId is required' });

  const videoId = rawVideoId.trim();
  if (!/^[a-zA-Z0-9_-]{11}$/.test(videoId)) return res.status(400).json({ error: 'Invalid YouTube video ID format' });
  ...
  try {
    ...
    if (!pageResponse.ok) {
      if (pageResponse.status === 429) return res.status(429).json({ error: 'YouTube is temporarily limiting requests. Try again in a few minutes.' });
    ...
    if (html.includes('consent.youtube.com') || html.includes('consent.google.com')) {
      return res.status(403).json({ error: 'YouTube requires consent verification. Please try a different video.' });
    }

    const playerMatch = html.match(/ytInitialPlayerResponse\s*=\s*(\{.+?\});/s);
    if (!playerMatch) return res.status(404).json({ error: 'This video is unavailable (may be private, deleted, or age-restricted).' });

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let playerResponse: any;
    try { playerResponse = JSON.parse(playerMatch[1]); } catch { return res.status(502).json({ error: 'Failed to parse YouTube player data' }); }

    if (playerResponse.playabilityStatus?.status && playerResponse.playabilityStatus.status !== 'OK') {
      return res.status(404).json({ error: 'This video is unavailable (may be private, deleted, or age-restricted).' });
    }

    const captionTracks = playerResponse.captions?.playerCaptionsTracklistRenderer?.captionTracks;
    if (!captionTracks || captionTracks.length === 0) return res.status(404).json({ error: 'No captions found for this video. Try a video with captions enabled.' });

    // Priority: manual EN > auto EN > manual any > auto any
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const manualEn = captionTracks.find((t: any) => t.languageCode?.startsWith('en') && t.kind !== 'asr');
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const autoEn = captionTracks.find((t: any) => t.languageCode?.startsWith('en') && t.kind === 'asr');
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const manualAny = captionTracks.find((t: any) => t.kind !== 'asr');
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const autoAny = captionTracks.find((t: any) => t.kind === 'asr');
    const track = manualEn || autoEn || manualAny || autoAny || captionTracks[0];

    const captionResponse = await fetch(track.baseUrl);
    if (!captionResponse.ok) return res.status(502).json({ error: 'Failed to download captions. Please try again.' });
    ...
    while ((match = segmentRegex.exec(captionXml)) !== null) {
      const text = match[3].replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/<[^>]+>/g, '').trim();
      if (text) segments.push({ start: parseFloat(match[1]), text });
    }

    if (segments.length === 0) return res.status(404).json({ error: 'Caption track was empty. Try a different video.' });

    // Truncate long transcripts to first 15 minutes
    let truncated = false;
    let filteredSegments = segments;
    if (segments.map(s => s.text).join(' ').length > 20000) {
      filteredSegments = segments.filter(s => s.start <= 900);
      truncated = true;
    }
    ...
    return res.status(200).json({
      ...
      transcript: filteredSegments.map(s => s.text).join(' '),
    ...
    captureError(err, { route: 'meta/youtube-transcript', organizationId: auth.organizationId });
    await flushSentry();
    return res.status(500).json({ error: 'Failed to extract transcript', message: err instanceof Error ? err.message : 'Unknown error' });
  }
}

// ─── Route: video-direct-start ───────────────────────────────────────────────

async function handleVideoDirectStart(req: VercelRequest, res: VercelResponse) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  const auth = await authenticateRequest(req);
  if (!auth) return res.status(401).json({ error: 'Unauthorized' });

  const { fileSize, title } = req.body || {};
  if (!fileSize || typeof fileSize !== 'number' || fileSize <= 0) return res.status(400).json({ error: 'fileSize (positive number) is required' });

  const creds = await loadCredentials(auth.organizationId);
  if (!creds) return res.status(404).json({ error: 'Meta credentials not found' });
  if (!creds.adAccountId) return res.status(400).json({ error: 'No ad account configured' });

  try {
    const startParams = new URLSearchParams({ upload_phase: 'start', file_size: String(fileSize), access_token: creds.accessToken, ...(title ? { title } : {}) });
    const startResponse = await fetch(`${GRAPH_API_BASE}/${creds.adAccountId}/advideos?${startParams.toString()}`, { method: 'POST' });
    const startData = await startResponse.json();

    if (!startResponse.ok || !startData.upload_session_id) {
      console.error('Meta video upload start failed:', startData);
      return res.status(502).json({ error: 'Failed to start Meta video upload', message: startData.error?.message || 'No upload_session_id returned' });
    }

    return res.status(200).json({ uploadSessionId: startData.upload_session_id });
  } catch (err: unknown) {
    console.error('Video direct start error:', err);
    captureError(err, { route: 'meta/video-direct-start', organizationId: auth.organizationId });
    await flushSentry();
    return res.status(500).json({ error: 'Video upload start failed', message: err instanceof Error ? err.message : 'Unknown error' });
  }
}

// ─── Route: video-direct-transfer ────────────────────────────────────────────

async function handleVideoDirectTransfer(req: VercelRequest, res: VercelResponse) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  const auth = await authenticateRequest(req);
  if (!auth) return res.status(401).json({ error: 'Unauthorized' });

  const { uploadSessionId, startOffset, chunkBase64 } = req.body || {};
  if (!uploadSessionId) return res.status(400).json({ error: 'uploadSessionId is required' });
  if (startOffset === undefined || typeof startOffset !== 'number') return res.status(400).json({ error: 'startOffset (number) is required' });
  if (!chunkBase64) return res.status(400).json({ error: 'chunkBase64 is required' });
  ...
  try {
    ...
    const multipartBody = Buffer.concat(parts);
    const transferResponse = await fetch(`${GRAPH_API_BASE}/${creds.adAccountId}/advideos`, {
      method: 'POST',
      headers: { 'Content-Type': `multipart/form-data; boundary=${boundary}`, 'Content-Length': String(multipartBody.length) },
      body: multipartBody,
    });
    const transferData = await transferResponse.json();
    ...
    if (!transferResponse.ok) {
      console.error('Meta video chunk transfer failed:', transferData);
      return res.status(502).json({ error: 'Failed to upload video chunk', message: transferData.error?.message || 'Chunk transfer failed' });
    }

    return res.status(200).json({ startOffset: transferData.start_offset, endOffset: transferData.end_offset });
  } catch (err: unknown) {
    console.error('Video direct transfer error:', err);
    captureError(err, { route: 'meta/video-direct-transfer', organizationId: auth.organizationId });
    await flushSentry();
    return res.status(500).json({ error: 'Video chunk transfer failed', message: err instanceof Error ? err.message : 'Unknown error' });
  }
}

// ─── Route: video-direct-finish ──────────────────────────────────────────────

async function handleVideoDirectFinish(req: VercelRequest, res: VercelResponse) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  const auth = await authenticateRequest(req);
  if (!auth) return res.status(401).json({ error: 'Unauthorized' });

  const { uploadSessionId, title } = req.body || {};
  if (!uploadSessionId) return res.status(400).json({ error: 'uploadSessionId is required' });
  ...
  try {
    const finishParams = new URLSearchParams({ upload_phase: 'finish', upload_session_id: uploadSessionId, access_token: creds.accessToken, ...(title ? { title } : {}) });
    const finishResponse = await fetch(`${GRAPH_API_BASE}/${creds.adAccountId}/advideos?${finishParams.toString()}`, { method: 'POST' });
    const finishData = await finishResponse.json();

    if (!finishResponse.ok || !finishData.video_id) {
      console.error('Meta video upload finish failed:', finishData);
      return res.status(502).json({ error: 'Failed to finalize Meta video upload', message: finishData.error?.message || 'No video_id returned' });
    }

    const videoId = finishData.video_id;

    // Poll for processing (max 60s)
    const maxPollTime = 60 * 1000;
    const pollStart = Date.now();
    let videoStatus = 'processing';

    while (Date.now() - pollStart < maxPollTime) {
      await new Promise(resolve => setTimeout(resolve, 5000));
      const statusResponse = await fetch(`${GRAPH_API_BASE}/${videoId}?fields=status&access_token=${creds.accessToken}`);
      const statusData = await statusResponse.json();
      videoStatus = statusData.status?.video_status || 'processing';
      if (videoStatus === 'ready') break;
      if (videoStatus === 'error') return res.status(502).json({ error: 'Meta video processing failed', video_id: videoId });
    }

    return res.status(200).json({ video_id: videoId, status: videoStatus });
  } catch (err: unknown) {
    console.error('Video direct finish error:', err);
    captureError(err, { route: 'meta/video-direct-finish', organizationId: auth.organizationId });
    await flushSentry();
    return res.status(500).json({ error: 'Video upload finish failed', message: err instanceof Error ? err.message : 'Unknown error' });
  }
}
