...
| `src/lib/customDirection.ts` | Operator creative brief for image generation — modes (`blend`/`override`), normalization, and the ONE definition of the prompt block both engines emit. Visual sibling of `manualSeed.ts` |
| `src/lib/angleScene.ts` | The ONE definition of what each creative ANGLE looks like — SUBJECT/MOMENT/COMPOSITION/LIGHT/AVOID scene grammar, the prompt block both image engines emit, and the video prompt's derived one-liner |
| `src/components/CustomDirectionField.tsx` | The brief input — collapsed by default, used at batch level (Blitz + single flow) and per image on the review step |
...
| `src/components/ShowcasePreview.tsx` | Live preview of the exact composite. The compositor is free browser canvas work, so configuring blind was a cost with no cause |
...
23. **The operator creative brief supersedes DERIVED DIRECTION, never the CONTRACT** — a user-written brief (`src/lib/customDirection.ts`) steers image generation for accounts with no ad history to learn from, or when the operator wants something deliberately different. It is the visual sibling of ADR #19's manual seed: both let a human supply evidence where the account has none.

    Two modes, one field. `blend` (default) layers the brief over ConversionIQ™'s derived direction and tells the model the brief wins on conflict. `override` REPLACES the derived direction — similarity ladder, `visualAnalysis`, top-ad descriptions, Ad Library inspiration, and the audience/intent *visual implication* lines — and withholds style references from the request entirely (`precomputeReferenceSet` returns `styleRefs: []` and skips the vision call). "Emulate these ads" and "do something completely different" cannot both be satisfied; asking for both yields a muddy compromise rather than either one.

    **The contract is never overridable**: product mockup identity lock, reference-image role labelling, aspect ratio, in-image text rules, the format-axis directive, and `IMAGE_SAFETY_DIRECTIVE`. This is enforced by ORDERING, not trust — the brief block is emitted at a single seam, after everything it may override and before everything it may not, so the binding rules are the last thing the model reads. A brief saying "ignore the product mockup" is still followed by the identity lock. Asserted in `imagePrompt.test.ts` by index comparison, not by substring presence.

    The brief block text lives in `customDirection.ts` and ONLY there; both engine builders import it, so a brief cannot be honoured on Gemini and dropped on gpt-image when `generateAdImage` fails over mid-batch. `customDirectionMode` on `GeneratedImageResult` is the one persisted fact about the brief's involvement — the review-card badge and "may this be presented as derived from proven winners" are DERIVED from it, exactly as ADR #20 does for `ReferenceSource`. Do not add a parallel boolean. The brief is deliberately NOT run through `POLICY_SANITIZE_PATTERNS`: those detect personal-attribute assertions in ad *copy*, and a scene description is not a claim made to a viewer.

24. **The angle axis must be visible in the picture, not just the caption** — a Blitz grid's angle reached copy generation and the Veo video prompt but never the image prompt: `regenerateAllImages` took one config for the whole pool, so every slot got a byte-identical request whose only per-image difference was "This is variation N of M — create a unique variation while maintaining brand consistency". A four-angle grid therefore came back as four re-lightings of one scene, and the "Pain Point" / "Transformation Promise" labels in the review step were captions `planBlitzImageSlots` attached to images that knew nothing about them.

    `src/lib/angleScene.ts` is the ONE definition of what an angle looks like. It replaced the thin `visualDirection` one-liner that used to sit on `CONCEPT_ANGLES` (an image model cannot place a "myth-vs-reality contrast" in a frame, so it fell back on the reference images — which are identical across slots, which *was* the convergence) with a scene grammar: SUBJECT, MOMENT, COMPOSITION, LIGHT and AVOID. `AVOID` does most of the work: every angle has a specific shot it collapses into when under-specified, and naming that collapse mode is what keeps the slots apart. `CONCEPT_ANGLES` now owns only the angle's *messaging* identity; `visualDirectionFor()` derives the video prompt's one-liner from the scene, so one angle can never be described as two different pictures.
...
26. **"Finished creative" is a STATE, not a source — and `asset_kind` is the one fact that records it** — the pixels ARE the ad, so nothing may generate, composite or emulate them. The app reached that state three ways before this was named: a Swipe Library image used as-is, a showcase composite, and (wrongly) a manual upload that became AI style direction. Naming it once and routing all three through `adType: 'showcase'` replaced three half-implementations with one.

    **It is not a fourth table.** Library tables are separated by EVIDENCE CLASS: `swipe_library_items` is own + measured, `inspiration_library_items` is external + unmeasured, `showcase_assets` is own + unmeasured. An operator-uploaded finished ad is own + unmeasured — the same class — so it is `asset_kind = 'finished'` on the existing table. A fourth table would split one evidence class in two and give the picker two places to look. `client_name` became nullable for the same reason: a creative for the operator's own offer has no client, and a NOT NULL would fill with placeholders until it meant nothing. Two `CHECK` constraints hold the invariants — a `source` asset must still name its client (a results wall labels cells with it), and a `finished` one may not carry a "before" (nothing composites it).

    **`as_is` is the only template that CONTAINS rather than covers.** Every other one frames a screenshot, which is a DOCUMENT — cropping discards the tail and keeps the proof. A finished creative is COMPOSED, so a crop cuts through an arrangement somebody made on purpose; letterboxing onto the palette is the only non-destructive answer when format and design disagree. And when they agree, `isPassthrough` detects that the plan would transform nothing and the stored bytes publish untouched — re-encoding a finished JPEG through canvas at q0.92 is a generation loss for no gain. That predicate reads the PLAN, not the template id, because `as_is` at a mismatched aspect genuinely does need the canvas.

    **The live preview exists because the composite is free.** Everything else in CreativeIQ costs a credit and a minute, so it is configured blind and reviewed after. A composite costs neither, so making the operator generate just to see what a template does is a cost with no cause — and it turns "will the screenshot be legible?" from an argument into something you look at. `loadShowcaseSources` and `showcaseConfigFrom` are split for this: pixels change only when the SELECTION changes, while the arrangement changes on every keystroke, and fusing them would refetch tens of MB per keypress.

    **Ingest scales by WIDTH and trims the overflow.** Bounding the longest edge starved the dimension that decides legibility — a 1440×4200 full-page capture came out 549px wide — and it spent that budget preserving height every template then cropped away. `planNormalize` is pure and unit-tested; the `reference` profile keeps fit-the-box semantics deliberately (a vision model wants the whole frame, and an 800×800 box is arithmetically identical to the old rule, so those callers are byte-identical).
...
- Don't make `fetch()` calls to Gemini (or any external AI API) without an `AbortController` timeout — without one, a hung connection blocks the UI for up to 5 minutes (browser default). Use 120s for image generation (reference images can be 25-50MB), 45s for text analysis. This caused perpetual loading in production (PR #271)
...
- `generateAdImageWithGemini`: 120s timeout per `fetch()` call — reference images can be 25-50MB, 60s was too tight
