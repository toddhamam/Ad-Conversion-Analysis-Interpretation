...
### What
A single AI provider was a single point of failure. When the OpenAI account ran out of credits, **every** AI path in the app died with `AI service error: You have no credits remaining` — including the manual seed analysis, which is exactly the cold-start path a new account depends on. There was no fallback: `/api/ai/chat` was a thin OpenAI passthrough, and all ~20 frontend call sites in `openaiApi.ts` are written against OpenAI's chat-completions shape, so swapping providers meant rewriting all of them.

This release makes `/api/ai/chat` **provider-agnostic**: it still accepts one OpenAI-shaped request, but can serve it from either OpenAI or Anthropic (Claude). Whichever provider runs first, the other is the **automatic fallback** when the first can't serve the request — exhausted credits, rate limit, invalid key, retired model, or outage. No frontend call site knows or cares which provider answered.

It also puts ConversionIQ™ **analysis and interpretation on Claude Fable 5**. These are infrequent, deep-reasoning runs that only fire when a meaningful batch of new ad data has landed, so the highest-reasoning tier is worth the per-call cost. Generation paths (copy, grid, creative) stay on GPT first — they run constantly and their prompts are tuned for it.

### Added
- **`api/_lib/anthropic-chat.ts`** — Claude chat helper that owns the entire dialect translation. Converts an OpenAI-shaped chat request into an Anthropic Messages API call and re-emits Anthropic's SSE stream as **OpenAI-shaped SSE**, so the existing stream reassembler in `openaiApi.ts` works unchanged. Translation rules:
  - `system` role messages → concatenated into Anthropic's top-level `system` field.
  - `image_url` with a `data:` URL → `{type:'image', source:{type:'base64', media_type, data}}` (media type normalized, e.g. `image/jpg` → `image/jpeg`); `http(s)` URLs → `{type:'image', source:{type:'url', url}}`.
  - `response_format: {type:'json_object'}` → JSON-only directive appended to `system` (Anthropic's structured outputs require a full schema, which these call sites don't supply).
  - `reasoning_effort` → `output_config.effort` (`none`/`low`→`low`, `medium`, `high`, `xhigh`).
  - `max_completion_tokens` → `max_tokens × 3`, capped at 64K — Claude Fable 5's thinking shares the output budget, so the OpenAI-sized budget would truncate long JSON analyses mid-object.
  - Anthropic `stop_reason` → OpenAI `finish_reason` (`max_tokens`→`length`, `refusal`→`content_filter`, else `stop`), plus `usage` mapped to `prompt_tokens`/`completion_tokens`/`total_tokens`.
- **Server-side refusal fallbacks on by default** — Claude Fable 5's safety classifiers can decline a request (HTTP 200 + `stop_reason: "refusal"`), which ad-creative analysis occasionally brushes against. Requests send `fallbacks: 'default'` with the `server-side-fallback-2026-07-01` beta so declines are transparently re-run on Anthropic's recommended model. If the org isn't enrolled in that beta the API rejects the parameter, so the helper detects that and retries once without it rather than failing the call.
- **`api/_lib/ai-chat.ts`** — the `/api/ai/chat` route handler, extracted out of `api/meta.ts` (which owns the Meta Graph API and had no business hosting an AI-provider router). Holds a `PROVIDERS` registry (`{ stream, isConfigured }` per provider), provider-order resolution, one-time request narrowing, the OpenAI passthrough, and the failover loop. **Adding a provider is now one table entry** — there is no `if (provider === 'openai')` branching left anywhere.
- **`api/_lib/sse-stream.ts`** — `SSEStream`, which sets SSE headers lazily on the first write so `started` means exactly "the response is committed". Replaces a `beginStream` callback plus two independently-maintained `started` flags plus a `res.headersSent` check that only worked because `setHeader()` was always immediately followed by a `write()`. The failover guard no longer depends on that Node timing detail.
- **`api/_lib/auth.ts`** — shared `authenticateRequest()` + `AuthContext`, previously duplicated byte-for-byte in `api/meta.ts` and `api/_lib/report-handlers.ts`. Both now import it.
- **`@anthropic-ai/sdk`** dependency (`^0.117.1`).
- **Env vars** — `ANTHROPIC_API_KEY` (required for the Claude path), plus optional `ANTHROPIC_ANALYSIS_MODEL` (defaults to `claude-fable-5`) and `AI_PRIMARY_PROVIDER` (account-wide `openai` | `anthropic` default). Documented in `.env.example` and `CLAUDE.md`.

### Changed
- **`/api/ai/chat` is now a multi-provider router.** Provider order resolves as: `ci_provider` on the request body → `AI_PRIMARY_PROVIDER` env var → OpenAI; unconfigured providers are filtered out. Providers throw a typed `ProviderError` (status + provider) rather than writing error responses themselves, and neither one closes the stream — the router owns the failover decision, the empty-response guard, and `end()`. `api/meta.ts` keeps a one-line `case 'ai-chat'` dispatch, matching how it already delegates to `report-handlers.ts`.
- **Failover rules** (`shouldTryNextProvider`) — retry the other provider on 401/402/403/404/408/409/429/5xx and network errors. **Do not** retry on 400/422: those are request-shape errors that would fail identically on both providers and only double the user's wait. Once bytes are on the wire the response is committed — no mid-stream failover.
- **ConversionIQ™ analysis routed to Claude first** (`openaiApi.ts`) — `analyzeAdCreative`, `analyzeCampaignAds`, `analyzeChannelPerformance`, and `distillManualAnalysis` (the manual seed analysis) pass `provider: ANALYSIS_PROVIDER` (`'anthropic'`). `callOpenAI` and `callOpenAIWithVision` gained an optional `provider` option that becomes the `ci_provider` routing hint on the request body; the backend strips it before forwarding upstream.
- **Error responses from `/api/ai/chat`** now report through Sentry (`captureError` + `flushSentry`) with the failing provider's status preserved, instead of a bare 500.

### Internal
- **Claude Fable 5 API constraints encoded in the helper**: thinking is always on (never send a `thinking` param), sampling params (`temperature`/`top_p`/`top_k`) are rejected, and assistant prefills are rejected. Fable 5 also requires 30-day data retention — it is unavailable under zero-data-retention org settings (set `ANTHROPIC_ANALYSIS_MODEL=claude-opus-5` if that applies).
- The SDK's `client.beta.messages.stream()` returns **synchronously** and issues the request lazily, so API errors surface while iterating rather than at the call site. The beta-rejection retry is structured around that — an early version that wrapped only the call site would never have caught the rejection.
- **Billing note**: `ANTHROPIC_API_KEY` is Anthropic **API** billing (console.anthropic.com). A Claude Pro/Max subscription cannot be used by a deployed server — separate products, separate billing.
- Verified against a **mock Anthropic endpoint** (no live key available in CI): 37 assertions covering the `SSEStream` lazy-header contract, system-message lifting, base64 + remote image translation, JSON-mode directive, effort and token-budget mapping, thinking-delta suppression, stop-reason and usage mapping, end-to-end reassembly by the real frontend parser, the beta-rejection retry path, classifier-decline handling, and the failover classification rules. All passing.
- **Net structural effect**: `api/meta.ts` **2552 → 2449 lines** — this PR adds a feature and leaves that file 103 lines *smaller* than it found it. `report-handlers.ts` drops back under 1k (1022 → 1000) via the auth dedup.
- `npm run build` (tsc + vite) clean; `api/` typechecks clean under `--strict`; new code is ESLint-clean, and the old `(response.body as any)` cast is gone (now `ReadableStream<Uint8Array>`). Pre-existing errors on untouched lines remain.
- Serverless function count unchanged at **11/12** — all four new modules live in `api/_lib/`, which Vercel does not count as routes. (CLAUDE.md previously claimed 12/12 and listed two `api/billing/*` routes that no longer exist; corrected in this PR.)
