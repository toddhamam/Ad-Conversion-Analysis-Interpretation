...
### Changed
- **`api/meta.ts` → `handleAIChat` is now a multi-provider router.** Provider order resolves as: `ci_provider` on the request body → `AI_PRIMARY_PROVIDER` env var → OpenAI; unconfigured providers are filtered out. The OpenAI passthrough was extracted into `streamOpenAIChat()`, which now throws a typed `ProviderError` (status + provider) instead of writing an error response directly, so the router can decide whether to retry elsewhere.
...
- Verified against a **mock Anthropic endpoint** (no live key available in CI): 25 assertions covering system-message lifting, base64 + remote image translation, JSON-mode directive, effort and token-budget mapping, stop-reason and usage mapping, end-to-end reassembly by the real frontend parser, the beta-rejection retry path, and the failover classification rules. All passing.
- `npm run build` (tsc + vite) clean; `api/` typechecks clean under `--strict`; new code is ESLint-clean (pre-existing errors on untouched lines in `openaiApi.ts` and `meta.ts` remain).
- Serverless function count unchanged at 12/12 — `anthropic-chat.ts` lives in `api/_lib/` (a shared helper, not a route).
