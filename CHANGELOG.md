...
## 2026-03-26 — Add YouTube transcript analysis and "From YouTube" copy source for authentic ad generation

### What
Full-stack feature: extract YouTube video transcripts, analyze them with GPT-5.4, and use the creator's authentic hooks and speaking style to generate high-converting Meta video and image ads. Adds "From YouTube" as a 5th copy source in CreativeIQ alongside Generate New, Import from Ads, Enter Manually, and From Swipe Library.
...
3. **"From YouTube" UI** — User pastes a YouTube URL in Step 1 → clicks Analyze → sees top hooks with timestamps and theme tags → pre-formatted copy options flow into Step 2 (copy selection) → generation uses authentic voice via prompt injection.
4. **Prompt Injection** — YouTube analysis is threaded through `generateAdPackage` → `regenerateAllImages` → `generateAdImage`/`generateAdImageWithGemini` and `generateAdVideoWithVeo`. Each variation rotates through extracted hooks so different ads use different proven moments.
5. **Chunked Video Upload** — Three new backend routes (`video-direct-start`, `video-direct-transfer`, `video-direct-finish`) implement Meta's chunked upload protocol for direct video file uploads with processing status polling.
...
- **src/pages/AdGenerator.tsx** — Added `'youtube'` to `CopySource` union type. Added YouTube state (URL, analysis, progress). Added `handleYouTubeAnalyze()` handler (URL validation, transcript fetch, AI analysis, copy option extraction). Added "From YouTube" button, URL input with Analyze action, progress indicator, analysis summary (top hooks with timestamps, theme tags). Threaded `youtubeAnalysis` into `generateAdPackage` only when `copySource === 'youtube'`. YouTube state clears when switching to other copy sources to prevent stale analysis leaking.
- **src/pages/AdGenerator.css** — Updated `.copy-source-options` grid from `repeat(4, 1fr)` to `repeat(5, 1fr)`. Updated tablet breakpoint to `repeat(3, 1fr)`.
